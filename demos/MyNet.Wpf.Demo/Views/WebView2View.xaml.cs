...
namespace MyNet.Wpf.Demo.Views
{
    /// <summary>
    ...
    public partial class WebView2View
    {
        ...
        {
            var text = await webView.ExecuteScriptAsync("quill.getText()").ConfigureAwait(false);
            resultTextBlock.Text = text;
        }

        private async void GetContentButton_ClickAsync(object sender, System.Windows.RoutedEventArgs e)
        {
            var text = await webView.ExecuteScriptAsync("quill.getContents()").ConfigureAwait(false);
            resultTextBlock.Text = text;
        }

        private async void GetHtmlButton_ClickAsync(object sender, System.Windows.RoutedEventArgs e)
        {
            var text = await webView.ExecuteScriptAsync("quill.container.innerHtml").ConfigureAwait(false);
            resultTextBlock.Text = text;
        }
    }
}
