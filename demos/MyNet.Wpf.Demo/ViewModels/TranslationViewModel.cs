...
namespace MyNet.Wpf.Demo.ViewModels
{
    ...
    {
        protected override string CreateTitle() => DemoResources.Translation;

        public ObservableCollection<StringTranslatable> UiResources { get; } = [];
        public ObservableCollection<StringTranslatable> FormatResources { get; } = [];
        public ObservableCollection<StringTranslatable> MessageResources { get; } = [];
        ...
        private static List<StringTranslatable> GetResources(ResourceManager resourceManager)
        {
            var resourceSet = resourceManager.GetResourceSet(GlobalizationService.Current.Culture, true, true);

            if (resourceSet is not null)
            {
                var result = new List<StringTranslatable>();
                foreach (DictionaryEntry entry in resourceSet)
                {
                    result.Add(new StringTranslatable(entry.Key.ToString().OrEmpty()));
                }
