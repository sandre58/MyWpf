...
namespace MyNet.Wpf.Demo.ViewModels
{
    ...
    {
        public AppointmentsListViewModel(DateTime startDate, DateTime endDate) : base([.. RandomGenerator.Int(500, 1000).Range().Select(x => new AppointmentSample($"Birthday n° {x}", RandomGenerator.Date(startDate, endDate).ToLocalTime(), new TimeSpan(RandomGenerator.Int(1, 8), RandomGenerator.Int(0, 59), 0)))])
