...
using MyNet.Wpf.Demo.TacticEditor;

namespace MyNet.Wpf.Demo.ViewModels
{
    internal class TacticEditorViewModel : NavigableWorkspaceViewModel
    {
        public IReadOnlyCollection<TacticPosition> TacticPositions { get; } = EnumClass.GetAll<Position>().Select(x => new TacticPosition(x)).OrderBy(x => x.Position).ToList().AsReadOnly();
