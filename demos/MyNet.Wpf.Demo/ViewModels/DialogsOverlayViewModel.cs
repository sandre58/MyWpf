...
using MyNet.Wpf.Demo.ViewModels.Contents;
    ...
    internal class DialogsOverlayViewModel : NavigableWorkspaceViewModel
    {
        ...
        public DialogsOverlayViewModel()
        {
            ...
            OpenFileDialogCommand = CommandsManager.Create<string>(async x =>
            {
                ...
                {
                    case "folder":
                        (result, path) = await DialogManager.ShowFolderBrowserDialogAsync(OpenFolderDialogSettings.Default).ConfigureAwait(false);
                        break;

                    case "file":
                        (result, path) = await DialogManager.ShowOpenFileDialogAsync().ConfigureAwait(false);
                        break;

                    case "images":
                        (result, path) = await ShowOpenImagesDialogAsync().ConfigureAwait(false);
                        break;

                    case "save":
                        (result, path) = await DialogManager.ShowSaveFileDialogAsync(SaveFileDialogSettings.Default).ConfigureAwait(false);
                        break;
                }
