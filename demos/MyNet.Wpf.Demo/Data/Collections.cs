...
namespace MyNet.Wpf.Demo.Data
{
    ...
    {
        public static readonly ImmutableList<int> Integers = [.. EnumerableHelper.Range(1, 100, 1)];
        ...
        [
            .. EnumClass.GetAll<Country>().GroupBy(x => x.Humanize()![..1])
                        .Select(x => new CountriesWrapper(x.OrderBy(x => x.GetDisplayName()), x.Key))
                        .OrderBy(x => x.DisplayName.Value)
,
        ];

        public static readonly ImmutableList<SelectedCountry> CountriesDetails = [.. EnumClass.GetAll<Country>().Select((x, index) => new SelectedCountry(x, index)).OrderBy(x => x.Item?.GetDisplayName())];
    }
}
