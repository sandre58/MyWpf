...
public class EnumSourceExtension : MarkupExtension
{
    ...
    public object? EnumsToExclude
    {
        ...
        set
        {
            ...
            var invalidEnumType = list.Select(v => Nullable.GetUnderlyingType(v.GetType()) ?? v.GetType()).FirstOrDefault(e => !e.IsEnum || e != EnumType);
    ...
    public override object? ProvideValue(IServiceProvider serviceProvider)
    {
        if (EnumType != null)
        {
            var enumValues = Enum.GetValues(EnumType).Cast<Enum>().Where(x => _enumsToExclude == null || !_enumsToExclude.Contains(x)).Select(x => new EnumTranslatable(x));
