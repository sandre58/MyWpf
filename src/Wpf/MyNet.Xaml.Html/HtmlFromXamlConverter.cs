...
public static class HtmlFromXamlConverter
{
    ...
    public static string ConvertXamlToHtml(string xamlString) => ConvertXamlToHtml(xamlString, true);
    ...
    public static string ConvertXamlToHtml(string xamlString, bool asFlowDocument)
    {
        ...
        using (var htmlStringWiter = new StringWriter(htmlStringBuilder, CultureInfo.InvariantCulture))
        {
    ...
    private static void WriteFormattingProperties(XmlTextReader xamlReader, XmlTextWriter htmlWriter, StringBuilder? inlineStyle)
    {
        ...
        _ = inlineStyle?.Remove(0, inlineStyle.Length);
        ...
        while (xamlReader.MoveToNextAttribute())
        {
            ...
            if (css != null)
            {
                _ = inlineStyle?.Append(css);
            }
        }

        if (borderSet)
        {
            _ = inlineStyle?.Append("border-style:solid;mso-element:para-border-div;");
        }

        // Return the xamlReader back to element level
        _ = xamlReader.MoveToElement();
        Debug.Assert(xamlReader.NodeType == XmlNodeType.Element);
    }
    ...
    private static string ParseXamlThickness(string thickness)
    {
        ...
        {
            values[i] = double.TryParse(values[i], NumberStyles.AllowDecimalPoint, CultureInfo.InvariantCulture, out var value)
                ? Math.Ceiling(value).ToString(CultureInfo.InvariantCulture)
                : "1";
        }
    ...
    private static void AddComplexProperty(XmlTextReader xamlReader, StringBuilder? inlineStyle)
    {
        ...
        {
            _ = inlineStyle.Append("text-decoration:underline;");
        }
        ...
        else
        {
            ...
            switch (xamlReader.Name)
            {
                ...
                case "List":
                    ...
                    htmlElementName = marker is null or "None" or "Disc" or "Circle" or "Square" or "Box"
                        ? "UL"
                        : "OL";
                    break;
                case "ListItem":
