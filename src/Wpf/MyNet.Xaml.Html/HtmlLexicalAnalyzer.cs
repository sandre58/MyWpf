...
internal class HtmlLexicalAnalyzer : IDisposable
{
        ...
        else
        {
            ...
            while (!IsAtTagStart && !IsAtEndOfStream && !IsAtDirectiveStart)
            {
                ...
                else
                {
                    ...
                    {
                        _ = _nextToken.Append(NextCharacter);
    ...
    private void GetNextCharacter()
    {
        ...
        if (NextCharacter == '&')
        {
            ...
            else if (char.IsLetter(_lookAheadCharacter))
            {
                ...
                {
                    _ = builder.Append(_lookAheadCharacter);
                    ReadLookAheadCharacter();
                }
                if (_lookAheadCharacter == ';')
                {
                    ...
                    if (HtmlSchema.IsEntity(builder.ToString()))
                    {
                        NextCharacter = HtmlSchema.EntityCharacterValue(builder.ToString());
    ...
    {
        if (_inputStringReader is not null)
        {
            _lookAheadCharacterCode = _inputStringReader.Read();
    ...
    private void ReadComment()
    {
        ...
        while (true)
        {
            ...
            else
            {
                ...
                _ = _nextToken.Append(_previousCharacter);
            }
        }
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (!_disposedValue)
        {
            if (disposing && _inputStringReader != null)
            {
