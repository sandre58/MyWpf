...
public abstract class FileExportByColumnsViewModelBase<T, TColumnMapping> : FileExportViewModelBase<T>
    where TColumnMapping : StringTranslatable
{
        ...
        : base(fileExtensionInfo, defaultExportName, defaultFolder)
    {
        ...
        Columns = [.. _defaultColumns.Select(x => new SelectedWrapper<TColumnMapping>(x.Key, x.Value))];
        ...
        Disposables.Add(Columns.ToObservableChangeSet(x => x.Id).WhenPropertyChanged(x => x.IsSelected).Subscribe(_ => OnPropertyChanged(nameof(AreAllSelected))));
    }
    ...
    protected override void ResetCore() => SetColumnsOrder([.. _defaultColumns.Keys.Select(x => x.Key)]);
}
