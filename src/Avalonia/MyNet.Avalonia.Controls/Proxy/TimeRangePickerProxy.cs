// -----------------------------------------------------------------------
// <copyright file="TimeRangePickerProxy.cs" company="Stéphane ANDRE">
...
public sealed class TimeRangePickerProxy : IControlProxy
{
    private readonly TimeRangePicker _control;

    public bool IsEmpty() => _control.StartTime is null && _control.EndTime is null;
    ...
    public TimeRangePickerProxy(TimeRangePicker control)
    {
        _control = control ?? throw new ArgumentNullException(nameof(control));
        _ = TimeRangePicker.StartTimeProperty.Changed.Subscribe(e =>
        {
            if (e.Sender is not TimeRangePicker dateTimePicker || dateTimePicker != _control)
                return;
            IsEmptyChanged?.Invoke(_control, EventArgs.Empty);
            IsActiveChanged?.Invoke(_control, EventArgs.Empty);
        });
        _ = TimeRangePicker.EndTimeProperty.Changed.Subscribe(e =>
        {
            if (e.Sender is not TimeRangePicker dateTimePicker || dateTimePicker != _control)
                return;
            IsEmptyChanged?.Invoke(_control, EventArgs.Empty);
