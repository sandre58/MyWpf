// -----------------------------------------------------------------------
// <copyright file="SmartHint.cs" company="Stéphane ANDRE">
...
public class SmartHint : Control
{
    ...
    static SmartHint() => DefaultStyleKeyProperty.OverrideMetadata(typeof(SmartHint), new FrameworkPropertyMetadata(typeof(SmartHint)));
    ...
    {
        if (dependencyObject is not SmartHint smartHint) return;
    ...
    private void RefreshState(bool useTransitions)
    {
        ...
        var action = new Action(() =>
        {
            ...
            state = UseFloating
                ? !proxy.IsEmpty() || proxy.IsFocused() ? HintFloatingPositionName : HintRestingPositionName
                : !proxy.IsEmpty() ? HintFloatingPositionName : HintRestingPositionName;
