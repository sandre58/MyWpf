...
internal class HtmlParser : IDisposable
{
    ...
    {
        var startHtmlIndex = htmlDataString.IndexOf("StartHTML:", StringComparison.OrdinalIgnoreCase);
        if (startHtmlIndex < 0)
        {
            return "ERROR: Urecognized html header";
        }

        startHtmlIndex = int.Parse(htmlDataString.AsSpan(startHtmlIndex + "StartHTML:".Length, "0123456789".Length));
        if (startHtmlIndex < 0 || startHtmlIndex > htmlDataString.Length)
        {
            return "ERROR: Urecognized html header";
        }

        var endHtmlIndex = htmlDataString.IndexOf("EndHTML:", StringComparison.OrdinalIgnoreCase);
        if (endHtmlIndex < 0)
        {
            return "ERROR: Urecognized html header";
        }

        endHtmlIndex = int.Parse(htmlDataString.AsSpan(endHtmlIndex + "EndHTML:".Length, "0123456789".Length));
        if (endHtmlIndex > htmlDataString.Length)
        {
            endHtmlIndex = htmlDataString.Length;
        }

        return htmlDataString[startHtmlIndex..endHtmlIndex];
    }
    ...
    internal static string AddHtmlClipboardHeader(string htmlString)
    {
        ...
        var startFragment = htmlString.IndexOf(HtmlStartFragmentComment, 0, StringComparison.OrdinalIgnoreCase);
        startFragment = startFragment >= 0 ? startHTML + startFragment + HtmlStartFragmentComment.Length : startHTML;
        var endFragment = htmlString.IndexOf(HtmlEndFragmentComment, 0, StringComparison.OrdinalIgnoreCase);
        endFragment = endFragment >= 0 ? startHTML + endFragment : endHTML;

        // Create HTML clipboard header string
        _ = stringBuilder.AppendFormat(CultureInfo.InvariantCulture, HtmlHeader, startHTML, endHTML, startFragment, endFragment, startFragment, endFragment)
                         // Append HTML body.
                         .Append(htmlString);
    ...
    private XmlElement ParseHtmlContent()
    {
        ...
        while (_htmlLexicalAnalyzer?.NextTokenType != HtmlTokenType.EOF)
        {
            if (_htmlLexicalAnalyzer?.NextTokenType == HtmlTokenType.OpeningTagStart)
            {
                ...
                {
                    var htmlElementName = _htmlLexicalAnalyzer.NextToken.ToLower(CultureInfo.InvariantCulture);
                    _htmlLexicalAnalyzer.GetNextTagToken();
        ...
        if (htmlRootElement.FirstChild is XmlElement element &&
            htmlRootElement.FirstChild == htmlRootElement.LastChild &&
            htmlRootElement.FirstChild.LocalName.Equals("html", StringComparison.CurrentCultureIgnoreCase))
        {
            htmlRootElement = element;
        }
    ...
    private void AddEmptyElement(XmlElement htmlEmptyElement)
    {
        ...
        _ = htmlParent.AppendChild(htmlEmptyElement);
    }
    ...
    private void OpenStructuringElement(XmlElement htmlElement)
    {
        ...
        if (_openedElements.Count > 0)
        {
            ...
            if (htmlParent != null)
            {
                ...
                _ = htmlParent.AppendChild(htmlElement);
            }
        }
    ...
    private void CloseElement(string htmlElementName)
    {
        ...
        if (_pendingInlineElements.Count > 0 && _pendingInlineElements.Peek().LocalName == htmlElementName)
        {
            ...
            InvariantAssert(_openedElements.Count > 0, "CloseElement: Stack of opened elements cannot be empty, as we have at least one artificial root element");
            var htmlParent = _openedElements.Peek();
            _ = htmlParent.AppendChild(htmlInlineElement);
        }
    ...
    private void AddTextContent(string textContent)
    {
        ...
        _ = htmlParent.AppendChild(textNode);
    }
    ...
    private void AddComment(string comment)
    {
        ...
        _ = htmlParent.AppendChild(xmlComment);
    }
    ...
    {
        while (_htmlLexicalAnalyzer?.NextTokenType is not (HtmlTokenType?)HtmlTokenType.EOF and
            not (HtmlTokenType?)HtmlTokenType.TagEnd and
            not (HtmlTokenType?)HtmlTokenType.EmptyTagEnd)
        {
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (!_disposedValue)
        {
            if (disposing && _htmlLexicalAnalyzer != null)
            {
                _htmlLexicalAnalyzer.Dispose();
                _htmlLexicalAnalyzer = null;
            }
