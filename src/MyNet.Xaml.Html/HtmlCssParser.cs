...
internal static class HtmlCssParser
{
                ...
                if (styleNameValue.Length == 2)
                {
                    ...
                    switch (styleName)
                    {
                        ...
                        case "margin": // top/right/bottom/left
                            _ = ParseCssRectangleProperty(styleValue, ref nextIndex, localProperties, styleName);
                            break;
                        case "margin-top":
                        case "margin-right":
                        case "margin-bottom":
    ...
    private static string? ParseCssSize(string styleValue, ref int nextIndex, bool mustBeNonNegative)
    {
        ...
        if (nextIndex < styleValue.Length && char.IsDigit(styleValue[nextIndex]))
        {
            ...
            var number = styleValue[startIndex..nextIndex];

            var unit = ParseWordEnumeration(FontSizeUnits, styleValue, ref nextIndex);
            unit ??= "px";

            return mustBeNonNegative && styleValue[startIndex] == '-' ? "0" : number + unit;
        }
    ...
    private static string? ParseCssColor(string styleValue, ref int nextIndex)
    {
        ...
        if (nextIndex < styleValue.Length)
        {
            ...
            else if (styleValue.Substring(nextIndex, 3).Equals("rbg", StringComparison.CurrentCultureIgnoreCase))
            {
    ...
    private static void ParseCssFontStyle(string styleValue, ref int nextIndex, Hashtable localProperties) => ParseWordEnumeration(FontStyles, styleValue, ref nextIndex, localProperties, "font-style");

    private static void ParseCssFontVariant(string styleValue, ref int nextIndex, Hashtable localProperties) => ParseWordEnumeration(FontVariants, styleValue, ref nextIndex, localProperties, "font-variant");

    private static void ParseCssFontWeight(string styleValue, ref int nextIndex, Hashtable localProperties) => ParseWordEnumeration(FontWeights, styleValue, ref nextIndex, localProperties, "font-weight");
    ...
    private static void ParseCssTextTransform(string styleValue, ref int nextIndex, Hashtable localProperties) => ParseWordEnumeration(TextTransforms, styleValue, ref nextIndex, localProperties, "text-transform");
    ...
    private static void ParseCssTextAlign(string styleValue, ref int nextIndex, Hashtable localProperties) => ParseWordEnumeration(TextAligns, styleValue, ref nextIndex, localProperties, "text-align");
    ...
    private static void ParseCssFloat(string styleValue, ref int nextIndex, Hashtable localProperties) => ParseWordEnumeration(Floats, styleValue, ref nextIndex, localProperties, "float");
    ...
    private static void ParseCssClear(string styleValue, ref int nextIndex, Hashtable localProperties) => ParseWordEnumeration(Clears, styleValue, ref nextIndex, localProperties, "clear");
    ...
    private static bool ParseCssRectangleProperty(string styleValue, ref int nextIndex, Hashtable localProperties, string propertyName)
    {
        ...
        Debug.Assert(propertyName is "margin" or "padding" or "border-width" or "border-style" or "border-color");
...
internal class CssStylesheet
{
    ...
    {
        if (htmlElement.LocalName.Equals("link", StringComparison.CurrentCultureIgnoreCase))
        {
            return;
        ...
        if (!htmlElement.LocalName.Equals("style", StringComparison.CurrentCultureIgnoreCase))
        {
            ...
            for (var htmlChildNode = htmlElement.FirstChild; htmlChildNode != null; htmlChildNode = htmlChildNode.NextSibling)
            {
                ...
                {
                    DiscoverStyleDefinitions(element);
                }
            }
            return;
        }
        ...
        for (var htmlChildNode = htmlElement.FirstChild; htmlChildNode != null; htmlChildNode = htmlChildNode.NextSibling)
        {
            if (htmlChildNode is XmlText or XmlComment)
            {
                _ = stylesheetBuffer.Append(RemoveComments(htmlChildNode!.Value!));
            }
        }
    ...
    private static string RemoveComments(string text)
    {
        ...
        return commentEnd < 0
            ? text[..commentStart]
            : text[..commentStart] + " " + RemoveComments(text[(commentEnd + 2)..]);
    }
    ...
    public string? GetStyle(string elementName, List<XmlElement> sourceContext)
    {
        ...
        Debug.Assert(elementName == sourceContext[^1].LocalName);
        ...
        if (_styleDefinitions != null)
        {
            for (var i = _styleDefinitions.Count - 1; i >= 0; i--)
            {
                ...
                if (MatchSelectorLevel(selectorLevel, sourceContext[^1]))
                {
    ...
    private static bool MatchSelectorLevel(string selectorLevel, XmlElement xmlElement)
    {
        ...
        if (indexOfDot >= 0)
        {
            if (indexOfDot > 0)
            {
                selectorTag = selectorLevel[..indexOfDot];
            }
            selectorClass = selectorLevel[(indexOfDot + 1)..];
        }
        else if (indexOfPound >= 0)
        {
            if (indexOfPound > 0)
            {
                selectorTag = selectorLevel[..indexOfPound];
            }
            selectorId = selectorLevel[(indexOfPound + 1)..];
        }
        else
        {
        ...
        return (selectorTag == null || selectorTag == xmlElement.LocalName)
&& (!(selectorId != null && HtmlToXamlConverter.GetAttribute(xmlElement, "id") != selectorId
) && selectorClass == null || HtmlToXamlConverter.GetAttribute(xmlElement, "class") == selectorClass);
    }

    private sealed class StyleDefinition(string selector, string definition)
    {
