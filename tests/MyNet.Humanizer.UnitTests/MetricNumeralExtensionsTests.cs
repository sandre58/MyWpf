// -----------------------------------------------------------------------
// <copyright file="MetricNumeralExtensionsTests.cs" company="Stéphane ANDRE">
...
public class MetricNumeralExtensionsTests
{
    ...
    public static TheoryData<int> SymbolRange => [.. Enumerable.Range(-24, 51)];
    ...
    public void FromMetric(double expected, string input) => Assert.Equal(expected, input.FromMetric());
    ...
    public void ToMetric(string expected, double input, bool hasSpace, bool useSymbol, int? decimals) => Assert.Equal(expected, input.ToMetric(hasSpace, useSymbol, decimals));
    ...
    [InlineData(-1E+27)]
    [InlineData(-1E-27)]
    public void ToMetricOnInvalid(double input) => Assert.Throws<ArgumentOutOfRangeException>(() => input.ToMetric());
}
